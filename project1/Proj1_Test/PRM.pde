...
java.util.HashSet<Integer>[] neighbors = new java.util.HashSet[maxNumNodes];  //A list of neighbors can can be reached from a given node
...
void connectNeighbors(Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  for (int i = 0; i < numNodes; i++){
    neighbors[i] = new java.util.HashSet<Integer>();  //Clear neighbors list
    for (int j = 0; j < numNodes; j++){
      if (i == j) continue; //don't connect to myself 
      Vec2 dir = nodePos[j].minus(nodePos[i]).normalized();
      float distBetween = nodePos[i].distanceTo(nodePos[j]);
      hitInfo circleListCheck = rayCircleListIntersect(centers, radii, numObstacles, nodePos[i], dir, distBetween, agentSize);
...
ArrayList<Integer> planPath(Vec2 startPos, Vec2 goalPos, Vec2[] centers, float[] radii, int numObstacles, Vec2[] nodePos, int numNodes){
  ...
  neighbors[numNodes] = new java.util.HashSet<Integer>();
  neighbors[numNodes+1] = new java.util.HashSet<Integer>();
  for (int i = numNodes; i < (numNodes+2); i++){
  ...
  // Run A*
  path = runAStar(nodePos, numNodes+2, numNodes, numNodes+1);
  return path;
}
...
ArrayList<Integer> runAStar(Vec2[] nodePos, int numNodes, int startID, int goalID){
  ...
  java.util.PriorityQueue<Node> fringe = new java.util.PriorityQueue<Node>(); //New empty fringe
  ArrayList<Integer> path = new ArrayList();
  ...
  Node startNode = new Node(startID, nodePos, goalID, 0);
  gScore[startID] = 0;
  ...
  while (fringe.size() > 0){
    ...
    for (int neighborNode : neighbors[currentNode]){
      if (!visited[neighborNode]){
        ...
        float new_gScore = curNode.g + nodePos[currentNode].distanceTo(nodePos[neighborNode]);
        if (new_gScore < gScore[neighborNode]) {
          parent[neighborNode] = currentNode;
          gScore[neighborNode] = new_gScore;
          if (!inQueue[neighborNode]) {
            fringe.add(new Node(neighborNode, nodePos, goalID, new_gScore));
            inQueue[neighborNode] = true;
          }
        }
